...
export const forgotPassword = catchAsync(async (request, response, next) => {
  ...
  try {
    const url = `${request.protocol}://${request.get('host')}/api/v1/users/reset-password/${resetToken}`;
...
export const changePassword = catchAsync(async (request, response, next) => {
  ...
  if (
    !user ||
    !(await User.isCorrectPassword(request.body.currentPassword, user.password))
  )
